...
class Mask:
        ...
        while True:  
            ...
            try:
                ...
                if len(refPT[counter]) > 1: # after two clicks
                    ima = cv2.line(ima, line[0][0], line[1][0], (255,255,255), thickness=2) 
                    cv2.imshow('Imagen', ima); cv2.waitKey(1)
    ...
    def create_mask(self):
        ...
        for i in range(n_slc):
            ...
            img_poly = cv2.polylines(img_copy, [poly], True, (255,255,255), thickness=3)
        ...
        save_nifti(Path('/'.join(self.study_subfolder.parts[:-1])) / 'mask', \
                    masks.astype(np.float32), affine)