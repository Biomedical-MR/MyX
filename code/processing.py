...
from dipy.reconst.dti import apparent_diffusion_coef, _nlls_err_func
...
###############################################################################
class MTProcessor:
    def __init__(self, mt_study_path: str, mask_path: str, path_mt='', path_m0='') -> None:
    ...
    def process_MT(self):
        ...
        if n_mt == 1:
            ...
            if len(mt_folders_list) == 1:
                ...
                f_mton_path = list(self.mt_study_path.glob(f'procesado_MT_*subscan_{subscan_index}.nii*'))[0]
                f_mtoff_path = list(self.mt_study_path.glob(f'procesado_M0_*subscan_{subscan_index}.nii*'))[0]
                ...
                for i in mt_folders_list:
                    ...
                    saving_path = os.path.join(str(self.mt_study_path), str(i), mt_map_filename)
                    save_nifti(saving_path, mt_map.astype(np.float32), affine1)
                    Heatmap().save_heatmap(mt_map, 'MT', out_path=os.path.join(str(self.mt_study_path), str(i)))
...
class DTIProcessor:
    ...
    def get_bvals_n_dirs(self, n_b_val, n_basal, n_dirs): 
        ...
        if n_b_val != n_b_val_real:
            ...
            print(f"Has especificado {n_b_val}, cuando se adquirieron {n_b_val_real}. Se va a continuar con el valor real.")
        ...
        if n_basal != n_basal_real:
            ...
            print(f"Has especificado {n_basal}, cuando se adquirieron {n_basal_real}. Se va a continuar con el valor real.")
                ...
                if n_dirs > n_dirs_real:
                    while True:
                        try:
                            ...
                            n_dirs = int(input(f"{hmg.ask}¿Número de direcciones?\n{hmg.pointer}"))
                            break
                        except ValueError:
                            print(f'{hmg.error}Debes introducir un número.')
                else:
        ...
        if n_dirs == n_dirs_real:
            ...
            new_dirs = ask_user(f"¿Deseas eliminar direcciones antes de continuar? En caso contrario, se continuará con el número original ({n_dirs_real}).")
            if new_dirs:
                while True:
                    try:
                        n_dirs_to_rm = int(input(f"\n{hmg.ask}¿Cuántas direcciones deseas eliminar?\n{hmg.pointer}"))
                        if (0 <= n_dirs_to_rm < n_dirs_real):
                            n_dirs = n_dirs_real - n_dirs_to_rm
                            break
                        else:
                            print(f'{hmg.error}Debes introducir un número entre 0 y {n_dirs_real}.')
                    except ValueError:
                        print(f'{hmg.error}Debes introducir un número.')
        else:
            new_dirs = True
                ...
                while True:
                    try:
                        ...
                        if (1 <= temp <= n_dirs_real) and (temp not in dirs_to_rm):
                            ...
                            print(f'{hmg.error}Debes introducir un número entre 1 y {n_dirs_real}.')
                    except ValueError:
                        print(f'{hmg.error}Debes introducir un número.')
    ...
    def get_residuals(self, design_matrix, data, weighting=None, sigma=None, jac=True):
        ...
        for vox in range(r_data.shape[0]):
            ...
            if jac:
                ...
                residuals[vox] = _nlls_err_func(start_params, 
                                                design_matrix, 
                                                r_data[vox], 
                                                weighting, 
                                                sigma)
    ...
    def process_DTI(self):
        ...
        b_vals, dirs, n_b_val, n_basal, n_dirs, indexes_to_rm = self.get_bvals_n_dirs(n_b_val, n_basal, n_dirs)  # Changed by Raquel: add args to function
        ...
        try:
            nii_fname = self.study_path / ((self.study_path.parts[-1])[3:] + '_subscan_0.nii.gz')
        ...
        try:
            ...
            mask, affine = load_nifti(Path('/'.join(self.study_path.parts[:-1])) / 'mask.nii')
        ...
        save_nifti(str(self.study_path / 'ADC_map'), ADC_maps.astype(np.float32), affine)
        ...
        if n_b_val > 1: 
            for d in range(0, n_dirs): 
                # residuals
                dir_res = residuals[:,:,:,(leap[d]+1):((leap[d])+(1+n_b_val))] 
        ...
        if apply_filter:
            ...
            for R2_map_path in R2_map_paths: 
                ...
                f_R2_maps.append(f_R2_map) # filtered R2 maps for all gradient directions
        else:
        ...
        save_nifti(os.path.join(md_saving_path, 'MD_map'), 
        ...
        save_nifti(os.path.join(AD_saving_path, 'AD_map'), 
        ...
        save_nifti(os.path.join(RD_saving_path, 'RD_map'), 
        ...
        save_nifti(os.path.join(FA_saving_path, 'FA_map'), 
...
###############################################################################
class TimeCollector:
    def __init__(self, root_path: str, studies_to_process: str, \
                    modals_to_process: list) -> None:
        self.root_path = root_path
    ...
    def read_times(self, subfolder, time_line_start: str, n_char_to_remove: int):
        ...
        try:
            method_path = subfolder / (subfolder.parts[-1][n_char_to_remove:] + '_method.txt')
            with open(method_path, 'r') as f:
                lines = f.readlines()
    ...
    def get_times_auto(self):
        ...
        for subfolder in self.studies_to_process:
            ...
            if study_name.startswith('T1_') and study_name[:2] in self.modals_to_process:
            ...
            elif study_name.startswith('T2_') and study_name[:2] in self.modals_to_process:
            ...
            elif study_name.startswith('T2E_') and study_name[:3] in self.modals_to_process:
    ...
    def get_TR(self):
        ...
        while True:
            try:
                n_tr = int(input("¿Cuántos tiempos de repetición se usaron " \
    ...
    def get_selected_time(self, selected_modal: str):
        ...
        modals = ['T1', 'T2', 'T2E'] 
        time_file_names = ['TiemposRepeticion.txt',
                        'TiemposEco.txt',
                        'TiemposEcoStar.txt']

        modal_idx = modals.index(selected_modal)
        file_name = time_file_names[modal_idx] # get file name (str) associated to modality
...
class TMapProcessor:
    def __init__(self, study_path: str, mask_path: str, n_cpu: int, \
                    fitting_mode='nonlinear') -> None:
        self.study_path = study_path
        self.mask_path = mask_path
    ...
    def process_T_map(self, time_paths): 
        ...
        for T_path in T_paths:
            ...
            # save T1/T2/T2E heatmap and filter by R^2 if required
                ...
                if apply_filter:
                    th = R2MapGenerator().select_threshold()
                    f_R2_map = R2MapGenerator().get_filtered_R2(R2_map_path, f'{method}_map', th) 
    ...
    def select_threshold(self):
        while True:
            try:
                ...
                print(f'El umbral debe ser un número entre 0 y 1. Has introducido "{th}".')
...
class Heatmap:
    ...
    def change_colormap(self):
        while True:
            ...
            try:
                ...
                print(f'Has introducido "{cmap_name}" y ese nombre no corresponde '
    ...
    def compute_heatmaps(self, maps: np.array, map_type: str, cmap, vmin: int, \
                        vmax: int, out_path='', ind=False, save=False):
        ...
        for slc_idx, new_map in enumerate(maps): 
            ...
            sns.heatmap(r_map, cmap=cmap, xticklabels=False, yticklabels=False, \
                        vmin=vmin, vmax=vmax, cbar_ax=cbar_ax, \
                        ax=ax[slc_idx]).invert_xaxis()
            ax[slc_idx].set_title(f'{map_type} slice {str(slc_idx)}')
            
            if ind == True:
                ind_fig = plt.figure(frameon=False, num=slc_idx + fig.number + 1) 
                sns.heatmap(r_map, cmap=cmap, xticklabels=False, yticklabels=False, \
                            vmin=vmin, vmax=vmax).invert_xaxis() 
                if save == save:
                    ind_fig.savefig(os.path.join(
                            out_path, 
                            map_type + '_slice_' + str(slc_idx+1) + '.png')
                            )
                    plt.close(ind_fig)
    ...
    def save_ADC_heatmap(self, f_ADC_maps:np.array, study_path:str):
        ...
        '''
        f_ADC_maps[f_ADC_maps==0.] = np.nan
        ...
        for num_dir, ADC_map_dir in enumerate(f_ADC_maps): 
            ...
            if num_dir == 0:
                ...
                self.compute_heatmaps(ADC_map_dir, 'ADC', cmap, vmin, vmax, \
                                        out_path, ind=False, save=False)
                ...
                Label(root, text='Mínimo', font=('calibre',10, 'bold')).grid(row=0, column=0)
                Label(root, text='Máximo', font=('calibre',10, 'bold')).grid(row=2, column=0)
                Label(root, text='Color', font=('calibre',10, 'bold')).grid(row=4, column=0)

                # creating entries for inputs
                color_min_entry = Entry(root,textvariable=color_min, font=('calibre',10,'normal'))
                color_max_entry = Entry(root,textvariable=color_max, font=('calibre',10,'normal'))
                colormap_entry = Entry(root,textvariable=colormap, font=('calibre',10,'normal'))
                ...
                def get_selection():
                    ...
                    self.compute_heatmaps(ADC_map_dir, 'ADC', colormap_val, \
                            color_min_val, color_max_val, ind=False, save=False)
                ...
                def close():
                    ...
                    plt.clf()
                    self.compute_heatmaps(ADC_map_dir, 'ADC', colormap_val, \
                        color_min_val, color_max_val, out_path, ind=True, save=True)
    ...
    def save_heatmap(self, maps: np.array, map_type: str, out_path: str, \
        ...
        # individual heatmaps
        self.compute_heatmaps(maps, map_type, cmap, vmin, vmax, \
                                out_path, ind=False, save=False)
        ...
        tk.Label(root, text='Mínimo', font=('calibre',10, 'bold')).grid(row=0, column=0)
        tk.Label(root, text='Máximo', font=('calibre',10, 'bold')).grid(row=2, column=0)
        tk.Label(root, text='Color', font=('calibre',10, 'bold')).grid(row=4, column=0)

        # creating entries for inputs
        color_min_entry = tk.Entry(root,textvariable=color_min, font=('calibre',10,'normal'))
        color_max_entry = tk.Entry(root,textvariable=color_max, font=('calibre',10,'normal'))
        colormap_entry = tk.Entry(root,textvariable=colormap, font=('calibre',10,'normal'))
        ...
        def get_selection():
            ...
            self.compute_heatmaps(maps, map_type, colormap_val, color_min_val, \
                                    color_max_val, ind=False, save=False)
        ...
        def close():
            ...
            plt.clf()
            self.compute_heatmaps(maps, map_type, colormap_val, color_min_val, \
                color_max_val, out_path, ind=True, save=True)
